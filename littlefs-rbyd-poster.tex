...
\title{\Large \bf \lfs+: rbyd B-trees: Efficient B-trees when RAM $\ll$
    block size}
...
This creates unique challenges for filesystem design: (1)~strictly
...
(1)~libraries are required to run in bounded RAM, limiting data-structures to
strictly tail-recursive algorithms, and (2)~storage block sizes frequently
exceed the device's available RAM, making the reading of a full block into RAM
impossible.
