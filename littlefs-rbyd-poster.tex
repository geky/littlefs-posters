...
With block sizes in the range of {$\sim$}128KiB--1MiB, CoW algorithms
are slow, and RAM-dependent algorithms impossible.
...
B-trees~\cite{btree} with \emph{rbyd trees}---%
...
manipulated incrementally without reading into RAM.

The original Dhara tree implements a radix tree, and while it is
possible to extend the emulated array into a B-tree, the resulting
B-tree is relatively inflexible.
Extend a traditional B-tree with sparse keys and order-statistic ranges
does not change the $O(b) \rightarrow O(b)$ runtime for block size $b$,
but attempting the same transformation on a Dhara B-tree increases the
runtime $O(\log b) \rightarrow O(b \log b)$.

To solve this, we need to extend the sparse and order-statistic
properties in to the Dhara tree itself.
...
mimicking a traditional red-black tree~\cite{TODO}.
...
Measuring simulated file write throughput shows improved performance
...
filesystem performance---focusing on throughput and latency, with comparison
